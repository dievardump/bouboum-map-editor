define(function() {
	var Editor = {
		...
		draw: function() {
			...
			for (i = y / h; i < iterY; i++) {
				for (j = x / w; j < iterX; j++) {
					if (typeof items[i] !== 'undefined' && typeof items[i][j] !== 'undefined') {
						item = items[i][j];
						if (item === 0) {
		...
		fill: function(fn) {
			...
			var items = this.getItems(),
				sizes = this.sizes,
				iterY = sizes.maps.h / sizes.
			default.h,
			iterX = sizes.maps.w / sizes.
			default.w;
		...
		setBlocs: function(blocs, type) {
			...
			blocs.forEach(function(item) {
				if (typeof items[item.y] !== 'undefined' && typeof items[item.y][item.x] !== 'undefined') {
